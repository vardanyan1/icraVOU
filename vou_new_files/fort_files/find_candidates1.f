...
c
      ...
      LOGICAL there,ok,found,catsrc,new_source
      common webprograms
      ok = .TRUE. 
      found = .FALSE.
      catsrc=.false.
...
700   continue
     ...
     &               .or. (catalog(1:7) == 'vlassql')) THEN ! poserr third parmaeter
...
c end PG
         ELSE IF (catalog(1:11) == 'chandracsc2') THEN
            ...
            is=ie
            ie=index(string(is+1:len(string)),',')+is
c            print *,' flux_chandra b,flux_chandra h,flux_chandra m,flux_chandra s',
c     &   flux_chandra(ichandra,4),flux_chandra(ichandra,2),flux_chandra(ichandra,3),flux_chandra(ichandra,1)
...
c end PG
     ...
     &           .or.  (catalog(1:4) == '4lac') .or. (catalog(1:7) == '4fgldr4') ) then
            ...
            if ((bigbind(igam) .eq. 1) .or. (namegam(igam)(1:5) /= '2BIGB')) then
               ...
               else if (catalog(1:8) == '4fgldr4') then
 ...
 99   CONTINUE
     ...
     &                poserr_radio(k),mjdavg,mjdavg,radio_type(k)
                     t(ifound)=k
c                     if (i == ifound-1) new_source = .TRUE.
...
c               print *,' nrep(sfound) ',nrep(sfound)
...
c               new_source = .TRUE.
     ...
     &                       dec_other(i),dist)
               IF (dist*3600. < max(poserr_radio(k),10.)) THEN !!!!!
                  IF (.NOT.catsrc) write(*,*) 'Catalogued sources :'
