...
c this program read the output from vo tool and make a input for find candidates
      implicit none
      integer*4 icat,i,is,ie,ia,it,in,out,iskip,is0
...
c            print *,' catalog(1:it-1) ',catalog(1:it-1)
            if (ia .ne. 0) then
               read(ra(1:2),'(i2)')rah
               read(ra(4:5),'(i2)')ram
               read(ra(7:ie-1),*)ras
            ...
            if (catalog(1:it-1) == '4fgldr4')  ie=is+30
...
c read the flux
     ...
     &           (catalog(1:it-1) == '4lacdr3') .or. (catalog(1:it-1) == '4fgldr4') ) then
               ie=index(value(1:len(value)),',')
