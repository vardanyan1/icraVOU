...
if fromWPEAK != '--':
   ...
   if (nup > 1):
      ...
      ax1.vlines(nupeak_Wpeak,ymin=nufnumin,ymax=nufnumax,linestyle='-',color='#66ff33',linewidth=4,zorder=10) 
      ax1.plot(nupeak_Wpeak,nufnumean,marker="s",markersize=4, markeredgecolor="none",markerfacecolor="#ff6699",zorder=11)
      if ((nup > 14.5) & (nup < 15.5)):
         ax1.plot(nupeak_Wpeak,nufnumax,marker="^",markersize=7, markerfacecolor="#66ff33",zorder=10)
      if (nup >= 15.5):
         ax1.plot(nupeak_Wpeak,nufnumax,marker="^",markersize=7, markeredgecolor="none", markerfacecolor="#66ff33",zorder=10)
         ax1.plot(nupeak_Wpeak,nufnumin,marker="v",markersize=6, markeredgecolor="none", markerfacecolor="#66ff33",zorder=10)
mnl, mxl = ax1.get_ylim()
yyst = 0.02
if fromBLAST != '--':
   ax1.axvline(nupeak_blast,ymin=0.35,ymax=0.98,linestyle=':',color='#b36b00',linewidth=2,zorder=10) 
   yye = yyst + 0.03
   yy = mnl*(mxl/mnl)**yye
   ax1.text(nupeak_blast*0.6,yy,r'BLAST $\nu_{\rm peak}$',fontsize=9,rotation='vertical',color = '#b36b00')
   yye = 0.38
   yy = mnl*(mxl/mnl)**yye
   ax1.plot(nupeak_blast,yy,marker="^",markersize=8, markeredgecolor="none", markerfacecolor="#b36b00",zorder=12)
   ax1.plot(nupeak_blast,yy*2,marker="^",markersize=8, markeredgecolor="none", markerfacecolor="#b36b00",zorder=12)
   yye = 0.98
   yy = mnl*(mxl/mnl)**yye
   ax1.plot(nupeak_blast,yy,marker="v",markersize=8, markeredgecolor="none", markerfacecolor="#b36b00",zorder=12)
   ax1.plot(nupeak_blast,yy/2,marker="v",markersize=8, markeredgecolor="none", markerfacecolor="#b36b00",zorder=12)
#
