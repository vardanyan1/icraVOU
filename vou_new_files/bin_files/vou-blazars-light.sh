...
if [ $SITE == 'vizier' ]; then
   ...
   cp ${HERE}/cats1.ini.orig ${HERE}/cats1.ini
   cp ${HERE}/cats2.ini.orig ${HERE}/cats2.ini
fi
#New
...
echo Running VOU-Blazars V2.00-Beta
echo
...
if [ "$light" != "y" -a "$light" != "Y" ]; then
   ...
   echo conesearch --db ${HERE}/cats1.ini --catalog RACS --ra $ranh --dec $decnh --radius $sfov --runit arcmin --columns default -o tmp/${pidnm}racs.1.csv >> tmp/${pidnm}vosearch.txt
...
if [ $SITE == 'vizier' ]; then
   echo conesearch --db ${HERE}/cats1.ini --catalog 4XMM-DR12 --ra $ranh --dec $decnh --radius $sfov --runit arcmin --columns default -o tmp/${pidnm}4xmmdr12.1.csv >> tmp/${pidnm}vosearch.txt
else
   echo conesearch --db ${HERE}/cats1.ini --catalog 4XMM-DR13 --ra $ranh --dec $decnh --radius $sfov --runit arcmin --columns default -o tmp/${pidnm}4xmmdr13.1.csv >> tmp/${pidnm}vosearch.txt
...
#     cat tmp/${pidnm}vosearch.txt
   ...
   echo conesearch --db ${HERE}/cats1.ini --catalog 4LACDR3 --ra $ranh --dec $decnh --radius $sfov --runit arcmin --columns default -o tmp/${pidnm}4lacdr3.1.csv >> tmp/${pidnm}vosearch.txt
...
#   echo conesearch --db ${HERE}/cats2.ini --catalog 3FGL --ra $ranh  --dec $decnh --radius $sfov --runit arcmin --columns default -o tmp/${pidnm}3fgl.1.csv >> tmp/${pidnm}vosearch.txt
   ...
   echo conesearch --db ${HERE}/cats2.ini --catalog MST12Y --ra $ranh  --dec $decnh --radius $sfov --runit arcmin --columns default -o tmp/${pidnm}mst12y.1.csv >> tmp/${pidnm}vosearch.txt
...
#   [ -z $pid ] && read -p "Found error when searhcing through VO. Type 'n' to skip and continue; otherwise, rerun the VO conesearch" runvo
   ...
   irunvo=${irunvo}+1
   echo
   echo VO search returned errors for one or more catalogues. Running conesearch again $irunvo
...
#  testing testing...
#      echo conesearch --db ${HERE}/cats2.ini --catalog PanSTARRS --ra $ranh --dec $decnh --radius $radint --runit arcmin --columns default -o tmp/${pidnm}gb6.i.csv >> tmp/${pidnm}vosearch.txt
...
# 99 for sed moed, -9999 for pulsar. -1111 for Gamma, -2222 for GRB, no number
      if  [ $typer -gt 10000 -o $typer -lt -40000 -o $typer -eq 99 -o $typer -eq -9999 ]; then
         ...
         if [ $nn = $source -o $source == sed -o $source == lcurve ]; then
            ...
            echo conesearch --db ${HERE}/cats2.ini --catalog PCCS30 --ra $rar --dec $decr --radius 3 --runit arcmin --columns default -o tmp/${pidnm}pccs30.$nn.2.csv >> tmp/${pidnm}vosearch.txt
            ...
            echo conesearch --db ${HERE}/cats2.ini --catalog PCCS545 --ra $rar --dec $decr --radius 3 --runit arcmin --columns default -o tmp/${pidnm}pccs545.$nn.2.csv >> tmp/${pidnm}vosearch.txt
            echo conesearch --db ${HERE}/cats2.ini --catalog PCCS857 --ra $rar --dec $decr --radius 3 --runit arcmin --columns default -o tmp/${pidnm}pccs857.$nn.2.csv >> tmp/${pidnm}vosearch.txt
            ...
            echo conesearch --db ${HERE}/cats2.ini --catalog UNWISE --ra $rar --dec $decr --radius 10 --runit arcsec --columns default -o tmp/${pidnm}unwise.$nn.2.csv >> tmp/${pidnm}vosearch.txt
...
#      python3.10 ~/app/aladin_error_map.py --ra $racand --dec $deccand --input_file_error_circles tmp/${pidnm}error_map.txt --out tmp/vou-aladin-error-py.html
...
#      ${BINF}/convert_sed tmp/${pidnm}Sed.txt tmp/${pidnm}Out4SedTool.txt tmp/${pidnm}Sed.csv
      ...
      if [ $runmode != l -a $plotsed != N ]; then
         ...
         python3.10 ${HERE}/VOU-plotSED.py --xaxis f --infile1 tmp/${pidnm}Sed.csv --outfile tmp/${pidnm}PySED.png --title 'Source nr. '$source' RA='$racand' Dec='$deccand --upperlimits 'yes' --erosita 'yes' --Show 'yes'
      fi
##############################################################
